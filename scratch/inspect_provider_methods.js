        ...
        const res = await Runtime.evaluate({
            expression: `
                (() => {
                    ...
                    const methods = Object.getOwnPropertyNames(proto).filter(k => typeof provider[k] === 'function');
                    const instanceKeys = Object.keys(provider).filter(k => typeof provider[k] === 'function');
                    
                    return JSON.stringify({
                        className: proto.constructor ? proto.constructor.name : 'Unknown',
                        methods,
                        instanceKeys,
                        pushUpdateString: provider.pushUpdate ? provider.pushUpdate.toString() : 'None'
                    }, null, 2);
                })()
            `,
            returnByValue: true
