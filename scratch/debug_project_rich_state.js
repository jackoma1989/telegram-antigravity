        ...
        const res = await Runtime.evaluate({
            expression: `
                (() => {
                    ...
                    let stateDetails = {};
                    if (pmf.richProjectsProvider && pmf.richProjectsProvider.state) {
                        const s = pmf.richProjectsProvider.state;
                        stateDetails = {
                            keys: Object.keys(s),
                            activeProjectUuid: s.activeProjectUuid,
                            projectsCount: s.projects ? s.projects.length : 0,
                            projects: s.projects ? s.projects.map(p => ({
                                uuid: p.uuid,
                                name: p.name,
                                label: p.label,
                    ...
                    return JSON.stringify(stateDetails, null, 2);
                })()
            `,
            returnByValue: true
        });

        console.log("RICH PROJECTS STATE:");
