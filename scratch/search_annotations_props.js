...
async function run() {
    try {
                ...
                (async () => {
                    ...
                    for (const src of bundles) {
                        try {
                            ...
                            // Let's find some occurrences of "annotations" or "mergeAnnotations" or "updateAnnotations"
                            let idx = 0;
                            const matches = [];
                            while (true) {
                                idx = text.indexOf('updateAnnotations', idx);
                                if (idx === -1) break;
                                matches.push(text.substring(idx - 250, idx + 350));
                                idx += 'updateAnnotations'.length;
        ...
        console.log("ANNOTATIONS USAGES:");
