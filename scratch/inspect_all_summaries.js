        ...
        const res = await Runtime.evaluate({
            expression: `
                (() => {
                    ...
                    const summaries = provider.latestSummaries && provider.latestSummaries.summaries ? provider.latestSummaries.summaries : {};
                    const summaryMap = {};
                    for (const k in summaries) {
                        if (summaries[k]) {
                            summaryMap[k] = {
                                trajectoryId: summaries[k].trajectoryId,
                                summary: summaries[k].summary
                            };
                        }
                    }
