...
async function run() {
    try {
                ...
                (async () => {
                    ...
                    for (const src of bundles) {
                        try {
                            ...
                            // Let's find some occurrences of "pushUpdate" and dump a few characters of context
                            let idx = 0;
                            const matches = [];
                            while (true) {
                                idx = text.indexOf('pushUpdate', idx);
                                if (idx === -1) break;
                                matches.push(text.substring(idx - 100, idx + 250));
                                idx += 'pushUpdate'.length;
                                if (matches.length > 8) break; // Limit
                            }
        ...
        console.log("PUSH UPDATE USAGES:");
