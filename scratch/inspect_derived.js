        ...
        const res = await Runtime.evaluate({
            expression: `
                (() => {
                    ...
                    let dsType = typeof ds;
                    if (ds) {
                        if (ds instanceof Map) dsType = 'Map';
                        else if (ds instanceof Set) dsType = 'Set';
                        else if (Array.isArray(ds)) dsType = 'Array';
                    }
                    
                    return JSON.stringify({
                        dsType,
                        dsKeys: ds && dsType === 'Map' ? Array.from(ds.keys()) : null,
                        dsSize: ds && (ds.size !== undefined) ? ds.size : null
                    }, null, 2);
                })()
            `,
            returnByValue: true
