...
for (const block of blocks) {
    ...
    try {
        ...
        const activeMatch = obj.allVisibleButtons.find(b => {
            const text = b.text.toLowerCase().replace(/[↵\n\r\t]/g, '').trim();
            return approvalButtonTexts.includes(text);
        });

        if (activeMatch) {
            console.log(`[${obj.time}] Clean Match Found:`, activeMatch);
            matchCount++;
        }
    } catch (e) {}
}

console.log("TOTAL CLEAN MATCHES FOUND:", matchCount);
