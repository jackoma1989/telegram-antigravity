...
async function run() {
    ...
    try {
        ...
        const newTitle = '能够发送 但接收不到你的信息 (已验证)';
        ...
        const res = await Runtime.evaluate({
            expression: `
                (async () => {
                    ...
                    const existingSummary = provider.latestSummaries && provider.latestSummaries.summaries 
                        ? provider.latestSummaries.summaries[targetId] 
                        : null;
                        
                    if (!existingSummary) {
                        return { error: "No existing summary found in latestSummaries for convo " + targetId };
                    }
                    
                    const realTrajectoryId = existingSummary.trajectoryId;
                    ...
                    try {
                        const writePayload = {
                            ...
                            summary: {
                                ...
                                summary: nTitle,
                                lastModifiedTime: existingSummary.lastModifiedTime
                            }
                        };
