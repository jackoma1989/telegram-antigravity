...
async function run() {
    try {
        ...
        const res = await Runtime.evaluate({
            expression: `
                (() => {
                    ...
                    if (!provider.latestSummaries || !provider.latestSummaries.summaries) return "No latestSummaries";
                    
                    const keys = Object.keys(provider.latestSummaries.summaries);
                    if (keys.length === 0) return "latestSummaries has no entries";
                    
                    const sampleKey = keys[0];
                    const sampleVal = provider.latestSummaries.summaries[sampleKey];
                    return JSON.stringify({
                        sampleKey,
                        valType: typeof sampleVal,
                        valKeys: sampleVal ? Object.keys(sampleVal) : null,
                        valSummaryType: sampleVal ? typeof sampleVal.summary : null,
                        valSummaryVal: sampleVal ? sampleVal.summary : null
                    }, null, 2);
                })()
            `,
            returnByValue: true
        ...
        console.log("LATEST SUMMARIES DUMP:");
