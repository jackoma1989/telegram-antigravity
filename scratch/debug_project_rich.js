...
async function run() {
    try {
        ...
        const res = await Runtime.evaluate({
            expression: `
                (() => {
                    ...
                    let richProjKeys = [];
                    let richProjState = null;
                    if (pmf.richProjectsProvider) {
                        richProjKeys = Object.keys(pmf.richProjectsProvider);
                        if (typeof pmf.richProjectsProvider.getState === 'function') {
                            try {
                                const s = pmf.richProjectsProvider.getState();
                                richProjState = {
                                    keys: Object.keys(s),
                                    activeProjectUuid: s.activeProjectUuid,
                                    projectsCount: s.projects ? s.projects.length : 0,
                                    firstProj: s.projects && s.projects[0] ? {
                                        uuid: s.projects[0].uuid,
                                        label: s.projects[0].label,
                                        name: s.projects[0].name
                                    } : null
                                };
                            } catch(e) {
                                richProjState = "Error getState: " + e.message;
                            }
                        }
                    }
        ...
        console.log("RICH PROJECTS DEEP DIVE:");
